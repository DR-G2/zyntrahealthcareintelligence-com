...
function fail(msg: string, status = 400, step?: string) {
  return new Response(JSON.stringify({ success: false, error: msg, step: step ?? null }), { status, headers: h });
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (roleErr) {
      console.error("[export] admin_roles query error:", roleErr.message);
      return fail("Failed to verify admin role", 500, step);
    }
    if (!roleRow) return fail("Admin access required", 403, step);

    console.log(`[export] Admin verified: ${callerEmail} (${roleRow.role})`);
    ...
    const userId = body.target_user_id || (claims.claims.sub as string);

    if (body.target_user_id && !isUUID(body.target_user_id)) {
    ...
    const now = new Date();
    if (range === "7d") {
      dateFilter = { from: new Date(now.getTime() - 7 * 864e5).toISOString(), to: now.toISOString() };
    ...
    let psychQuery = supabase.from("psychograph_history").select("*").eq("user_id", userId).order("created_at", { ascending: false }).limit(500);
    if (dateFilter) psychQuery = psychQuery.gte("created_at", dateFilter.from).lte("created_at", dateFilter.to);
    ...
    let stationQuery = supabase
      ...
      .order("created_at", { ascending: false })
      .limit(2000);
    if (dateFilter) stationQuery = stationQuery.gte("created_at", dateFilter.from).lte("created_at", dateFilter.to);
    ...
    const performanceTrends = Object.entries(dailyMap)
      ...
      .map(([date, s]) => ({ date, accuracy: Math.round((s.correct / s.total) * 100), total_questions: s.total, avg_time_seconds: Math.round(s.time / s.total) }));
    ...
    const exportData = {
      ...
      clinical_reasoning_nodes: {
        ...
        psychograph_snapshots: (psychographs || []).slice(0, 50).map(p => ({ session_id: p.session_id, archetype: p.archetype, structure_integrity: p.structure_integrity, delegation_confidence: p.delegation_confidence, silence_tolerance: p.silence_tolerance, time_compression_vulnerability: p.time_compression_vulnerability, emotional_reactivity: p.emotional_reactivity, cognitive_stability: p.cognitive_stability, created_at: p.created_at })),
      },
      rule_out_patterns: {
        change_rate: totalAttempts > 0 ? Math.round(((attempts || []).filter(a => a.answer_changes_count > 0).length / totalAttempts) * 100) : 0,
        avg_changes_per_question: totalAttempts > 0 ? Math.round(((attempts || []).reduce((s, a) => s + a.answer_changes_count, 0) / totalAttempts) * 100) / 100 : 0,
      },
      question_history: (attempts || []).map(a => ({ question_id: a.question_id, is_correct: a.is_correct, selected_answer: a.selected_answer, time_taken_seconds: a.time_taken_seconds, answer_changes_count: a.answer_changes_count, change_sequence: a.change_sequence, question_position: a.question_position, time_to_first_click: a.time_to_first_click, category: (a.questions as any)?.category || null, difficulty: (a.questions as any)?.difficulty || null, subtopic: (a.questions as any)?.subtopic || null, created_at: a.created_at })),
      osce_history: (stationAttempts || []).map(sa => ({ session_id: sa.session_id, subject: sa.subject, scores: sa.scores, behavioral_signals: sa.behavioral_signals, psychograph: sa.psychograph, time_taken_seconds: sa.time_taken_seconds, mode: sa.mode, station_index: sa.station_index, created_at: sa.created_at })),
      mistake_patterns: mistakePatterns,
      performance_trends: performanceTrends,
      export_metadata: { exported_at: now.toISOString(), range, date_filter: dateFilter, platform: "zyntra" },
    };

    // ── Log export ──
    step = "log_export";
    const { data: lastVersion } = await supabase.from("data_export_history").select("version").eq("user_id", userId).order("version", { ascending: false }).limit(1).maybeSingle();
    ...
    await supabase.from("data_export_history").insert({
      user_id: userId, action_type: "export", file_name: fileName, version: nextVersion,
      snapshot_data: { range, format, record_counts: { attempts: (attempts || []).length, stations: (stationAttempts || []).length } },
    });

    console.log(`[export] Complete for user ${userId}, version ${nextVersion}`);
    ...
    return new Response(JSON.stringify(exportData, null, 2), { headers: h });

  } catch (e) {
    const msg = e instanceof Error ? e.message : "Unknown error";
    console.error(`[export] Error at step "${step}":`, msg);
    return new Response(JSON.stringify({ success: false, error: msg, step }), { status: 500, headers: h });
  }
});
